          ...
          {featuredWorks.map((work, index) => (
            <SwiperSlide key={work.id} className='relative'>
              ...
              <motion.div 
                ...
                className="flex flex-col absolute bottom-0 left-0 gap-2 xl:gap-2 pl-5 xl:pl-10 pb-10 md:pb-16">
                  <motion.h3
                  variants={slideIn('up', 'tween', index * 1, 1)} 
                  className="font-unbounded font-normal text-[16px] md:text-[20px] xl:text-[24px]">{work.title}</motion.h3>
                  <motion.p
                  variants={slideIn('up', 'tween', index * 1, 1)} 
                  className="font-montserrat text-[10px] md:text-[12px] xl:text-[16px] font-light">{work.subtitle}</motion.p>
                  <motion.p
                  variants={slideIn('up', 'tween', index * 1, 1)} 
                  className="font-montserrat text-[10px] md:text-[12px] xl:text-[16px] font-light">{work.year}</motion.p>
              </motion.div>
            </SwiperSlide>
          ))}
        </Swiper>
